...
char* smokertypes[3] = { "matches & tobacco", "matches & paper", "tobacco & paper" };
...
void* smoker(void* arg)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		printf("\033[0;37mSmoker %d \033[0;31m>>\033[0m Waiting for %s\n",
			smoker_id, smokertypes[type_id]);

		// Wait
		sem_wait(&smoker[type_id]);
...
void* pusher(void* arg)
{
	...
	for (int i = 0; i < 12; ++i)
	{
		sem_wait(&pusher[pusher_id]);
		sem_wait(&pusher);

		if (itemsontable[(pusher_id + 1) % 3])
		{
			itemsontable[(pusher_id + 1) % 3] = false;
			sem_post(&smoker[(pusher_id + 2) % 3]);
		}
		else if (itemsontable[(pusher_id + 2) % 3])
		{
			itemsontable[(pusher_id + 2) % 3] = false;
			sem_post(&smoker[(pusher_id + 1) % 3]);
		}
		else
		{
			itemsontable[pusher_id] = true;
		}
...
void* agent(void* arg)
{
	...
	for (int i = 0; i < 6; ++i)
	{
		usleep(rand() % 200000);
		sem_wait(&ready);
		sem_post(&pusher[agent_id]);
		sem_post(&pusher[(agent_id + 1) % 3]);
		printf("\033[0;35m==> \033[0;33mAgent %d giving out %s\033[0;0m\n",
			agent_id, smokertypes[(agent_id + 2) % 3]);
	}
